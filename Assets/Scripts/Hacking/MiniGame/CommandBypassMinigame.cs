...
public class CommandBypassMinigame : HackingMinigameBase
{
    ...
    private int totalKeysNeeded = 20;         // 정해진 개수만큼 맞추면 성공
    private const int arrowsPerSequence = 5;  // 한 번에 표시할 화살표 개수
    private const float preMadeArrowFontSize = 56f;
    ...
    /// </summary>
    public void SetPreMadeUI(Transform arrowContainer, Slider progress = null, Image progressFill = null, Slider timeSlider = null)
    {
        ...
        // 자식 슬롯 모드(Arrow_0~N): 우선 처리
        usePreMadeUI = arrowContainer.childCount >= arrowsPerSequence;
        if (usePreMadeUI)
        {
            CachePreMadeArrowTexts();
            return;
        }

        // ArrowContainer 자체에 Text/TMP 하나만 붙인 단일 텍스트 모드
    ...
    private void CachePreMadeArrowTexts()
    {
        ...
        for (int i = 0; i < preMadeArrowContainer.childCount; i++)
        {
            ...
            else
            {
                ...
                preMadeArrowTexts.Add(txt); // null이어도 슬롯 인덱스 유지
            }
        }
    }
    ...
    private void GenerateNewSequence()
    {
        ...
        // 랜덤한 방향키 시퀀스 생성 (항상 5개)
        int sequenceLength = arrowsPerSequence;
    ...
    {
        if (usePreMadeUI && preMadeArrowContainer != null && preMadeArrowContainer.childCount >= arrowsPerSequence)
        {
    ...
    private void DisplaySequencePreMade()
    {
        ...
        // 자식 슬롯 모드: 현재 시퀀스(5개)만 표시
        int maxSlots = Mathf.Min(preMadeArrowContainer.childCount, preMadeArrowTexts.Count);
        for (int i = 0; i < maxSlots; i++)
        {
            Transform child = preMadeArrowContainer.GetChild(i);
            child.gameObject.SetActive(i < currentSequence.Count);

            // 슬롯 크기/정렬을 강제해 작은 글자/줄바꿈 레이아웃 깨짐 방지
            var rt = child as RectTransform;
            if (rt != null && rt.sizeDelta.sqrMagnitude < 1000f)
            ...
            if (i < currentSequence.Count && i < preMadeArrowTexts.Count)
            {
                ...
                if (tmp != null)
                {
                    tmp.text = symbol;
                    tmp.color = c;
                    tmp.fontSize = preMadeArrowFontSize;
                    tmp.alignment = TextAlignmentOptions.Center;
                    tmp.enableWordWrapping = false;
                    tmp.overflowMode = TextOverflowModes.Overflow;
                }
                else if (preMadeArrowTexts[i] is UnityEngine.UI.Text tx)
                {
                    tx.text = symbol;
                    tx.color = c;
                    tx.fontSize = Mathf.RoundToInt(preMadeArrowFontSize);
                    tx.alignment = TextAnchor.MiddleCenter;
                    tx.horizontalOverflow = HorizontalWrapMode.Overflow;
                    tx.verticalOverflow = VerticalWrapMode.Overflow;
                }
            }
            else if (i < preMadeArrowTexts.Count)
            {
                // 다음 시퀀스에서 이전 문자열/색상 잔상 제거
                var tmp = preMadeArrowTexts[i] as TextMeshProUGUI;
                if (tmp != null) { tmp.text = ""; tmp.color = Color.white; }
                else if (preMadeArrowTexts[i] is UnityEngine.UI.Text tx) { tx.text = ""; tx.color = Color.white; }
            }
        }
    }
    ...
    private void ClearCommandArrows()
    {
        if (usePreMadeUI && preMadeArrowContainer != null)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < preMadeArrowTexts.Count; i++)
                {
                    var tmp = preMadeArrowTexts[i] as TextMeshProUGUI;
                    if (tmp != null) { tmp.text = ""; tmp.color = Color.white; }
                    else if (preMadeArrowTexts[i] is UnityEngine.UI.Text tx) { tx.text = ""; tx.color = Color.white; }
    ...
    private void Update()
    {
        ...
        UpdateTimeGauge(); // 매 프레임 시간 게이지 업데이트
        ...
        if (currentInputIndex < currentSequence.Count)
        {
            ...
            if (Input.GetKeyDown(expectedKey))
            {
                ...
                DisplaySequence();
                UpdateProgressGauge(); // 진행도 게이지 업데이트
    ...
    public override void OnSuccess()
    {
        ...
        if (timeGauge != null) timeGauge.value = 1f; // 성공 시 시간 게이지 가득
        base.OnSuccess();
    }
    ...
    {
        isActive = false;
        if (timeGauge != null) timeGauge.value = 0f; // 실패 시 시간 게이지 비움
        base.OnFailure();
    }
}
