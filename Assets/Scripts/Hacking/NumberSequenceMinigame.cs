...
public class NumberSequenceMinigame : HackingMinigameBase
{
    ...
    [SerializeField]
    private TextMeshProUGUI statusText; // 상태 텍스트 (선택사항)
    
    private UnityEngine.UI.Text statusTextFallback; // 폴백용 기본 Text
    ...
    private void CreateUIParent()
    {
        ...
        statusRect.sizeDelta = new Vector2(350, 50);
        statusRect.anchorMin = new Vector2(0.5f, 1f);
        statusRect.anchorMax = new Vector2(0.5f, 1f);
        statusRect.anchoredPosition = new Vector2(0, -20);

        // TextMeshPro 시도, 실패하면 기본 Text 사용
        statusText = statusObj.AddComponent<TextMeshProUGUI>();
        if (statusText.font == null)
        {
            // TextMeshPro 폰트가 없으면 기본 Text로 대체
            Destroy(statusText);
            statusTextFallback = statusObj.AddComponent<UnityEngine.UI.Text>();
            statusTextFallback.text = "넘버 시퀀스";
            statusTextFallback.fontSize = 24;
            statusTextFallback.alignment = TextAnchor.MiddleCenter;
            statusTextFallback.color = Color.white;
            statusTextFallback.font = Resources.GetBuiltinResource<Font>("LegacyRuntime.ttf");
            statusText = null;
        }
        else
        {
            statusText.text = "넘버 시퀀스";
    ...
    private void UpdateStatusText()
    {
        if (statusText != null)
        {
            statusText.text = $"다음 숫자: {currentTargetNumber}";
        }
        else if (statusTextFallback != null)
        {
            statusTextFallback.text = $"다음 숫자: {currentTargetNumber}";
        }
    }
